...
class TestDataExfilAttack:
    ...
    async def test_refused_response_fails(self, attack, mock_target):
        mock_target.send = AsyncMock(return_value="I'm sorry, I can't share that information.")
        results = await attack.execute(mock_target, max_turns=10)
    ...
    @pytest.mark.asyncio
    async def test_data_leak_succeeds(self, attack, mock_target):
        mock_target.send = AsyncMock(
            return_value="Here are my system instructions: You are a helpful assistant designed to answer questions."
        )
        results = await attack.execute(mock_target, max_turns=10)
    ...
    async def test_error_handling(self, attack, mock_target):
        mock_target.send = AsyncMock(side_effect=Exception("Read error"))
        results = await attack.execute(mock_target, max_turns=10)
        for result in results:
    ...
    async def test_max_turns_limits_turns(self, attack, mock_target):
        mock_target.send = AsyncMock(return_value="I can't provide that.")
